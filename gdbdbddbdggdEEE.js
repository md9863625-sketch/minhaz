...
async function fetchSnippet(videoId) {
    const endpoint = `${WORKER_BASE}/videos?id=${encodeURIComponent(videoId)}`;
    const res = await fetch(endpoint);
    if (!res.ok) throw new Error('API Error');
    const json = await res.json();
    if (!json.items || !json.items.length) throw new Error('Video not found');
...
btnGet.addEventListener('click', async () => {
    ...
    try {
        ...
        const snippet = await fetchSnippet(vid);
        renderResults(snippet.tags || []);
    } catch (err) {
...
btnReset.addEventListener('click', () => {
    ...
    // 2. Clear the output box contents and reset its state
    outputBox.innerHTML = '';
    outputBox.style.display = 'none';
    outputBox.classList.remove("grid-active");

    // 3. Hide the notification bar
    noticeEl.style.display = 'none';

    // 4. Remove the Copy button from the DOM
    const oldBtn = document.getElementById('copy');
    if (oldBtn) oldBtn.remove();
    
    // 5. Ensure the Get button is enabled
    btnGet.disabled = false;
});